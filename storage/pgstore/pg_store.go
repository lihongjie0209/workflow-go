// Package pgstore provides a PostgreSQL-backed implementation of the storage.Store
// interface using pgx/v5 via database/sql.
package pgstore
...
// WithConnString sets the PostgreSQL connection string.
...
func WithConnString(connStr string) Option {
	return func(s *Store) {
		...
		if err != nil {
			panic(fmt.Sprintf("pgstore: failed to open db: %v", err))
		}
	}
}

// New creates a new PostgreSQL store with the given options.
func New(opts ...Option) *Store {
	...
	if s.db == nil {
		panic("pgstore: connection string is required (use WithConnString)")
	}
	if err := s.init(); err != nil {
		panic(fmt.Sprintf("pgstore: init failed: %v", err))
	}
	return s
}
...
func (s *Store) CreateProcessDefinition(ctx context.Context, def *spec.ProcessDefinition) error {
	...
	if err != nil {
		return fmt.Errorf("pgstore: create definition %q: %w", def.ID, err)
	}
	return nil
}
...
func (s *Store) DeleteProcessDefinition(ctx context.Context, id string) error {
	...
	if n == 0 {
		return fmt.Errorf("pgstore: process definition %q not found: %w", id, storage.ErrNotFound)
	}
	return nil
}
...
func (s *Store) UpdateProcessInstance(ctx context.Context, pi *instance.ProcessInstance) error {
	...
	n, _ := res.RowsAffected()
	if n == 0 {
		return fmt.Errorf("pgstore: process instance %q not found: %w", pi.ID, storage.ErrNotFound)
	}
	return nil
}
...
func (s *Store) GetProcessInstance(ctx context.Context, id string) (*instance.ProcessInstance, error) {
	...
	err := s.db.QueryRowContext(ctx,
		`SELECT def_id, state, variables, started_at, ended_at, parent_process_instance_id, parent_activity_id FROM process_instances WHERE id = $1`, id).
		Scan(&defID, &stateStr, &varsJSON, &startedAt, &endedAt, &parentPIID, &parentActID)
	if err == sql.ErrNoRows {
		return nil, fmt.Errorf("pgstore: process instance %q not found: %w", id, storage.ErrNotFound)
	}
	if err != nil {
		return nil, err
	}
...
func (s *Store) ListProcessInstances(ctx context.Context, defID string) ([]*instance.ProcessInstance, error) {
	rows, err := s.db.QueryContext(ctx,
		`SELECT id, def_id, state, variables, started_at, ended_at, parent_process_instance_id, parent_activity_id FROM process_instances WHERE def_id = $1`, defID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *Store) UpdateActivityInstance(ctx context.Context, ai *instance.ActivityInstance) error {
	...
	n, _ := res.RowsAffected()
	if n == 0 {
		return fmt.Errorf("pgstore: activity instance %q not found: %w", ai.ID, storage.ErrNotFound)
	}
	return nil
}
...
func (s *Store) CreateToken(ctx context.Context, t *instance.Token) error {
	...
	if err != nil {
		return fmt.Errorf("pgstore: create token %q: %w", t.ID, err)
	}
	return nil
}
...
func (s *Store) UpdateToken(ctx context.Context, t *instance.Token) error {
	...
	n, _ := res.RowsAffected()
	if n == 0 {
		return fmt.Errorf("pgstore: token %q not found: %w", t.ID, storage.ErrNotFound)
	}
	return nil
}
...
func (s *Store) DeleteToken(ctx context.Context, id string) error {
	...
	n, _ := res.RowsAffected()
	if n == 0 {
		return fmt.Errorf("pgstore: token %q not found: %w", id, storage.ErrNotFound)
	}
	return nil
}
...
func (s *Store) GetVariable(ctx context.Context, processInstanceID, name string) (any, error) {
	...
	if err == sql.ErrNoRows {
		return nil, fmt.Errorf("pgstore: variable %q not found for instance %q: %w", name, processInstanceID, storage.ErrNotFound)
	}
	if err != nil {
		return nil, err
	}
...
func (s *Store) DeleteVariable(ctx context.Context, processInstanceID, name string) error {
	...
	n, _ := res.RowsAffected()
	if n == 0 {
		return fmt.Errorf("pgstore: variable %q not found for instance %q: %w", name, processInstanceID, storage.ErrNotFound)
	}
	return nil
}
...
func (s *Store) ListCompletedProcessInstances(ctx context.Context, limit int) ([]*instance.ProcessInstance, error) {
	rows, err := s.db.QueryContext(ctx,
		`SELECT id, def_id, state, variables, started_at, ended_at, parent_process_instance_id, parent_activity_id FROM process_instances WHERE state = 'completed' ORDER BY ended_at DESC LIMIT $1`, limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *Store) QueryDefinitions(ctx context.Context, q storage.DefQuery) ([]*spec.ProcessDefinition, int, error) {
	...
	argIdx := 1
	if q.Key != "" {
		where += fmt.Sprintf(" AND key = $%d", argIdx); argIdx++
...
func (s *Store) QueryProcessInstances(ctx context.Context, q storage.InstQuery) ([]*instance.ProcessInstance, int, error) {
	...
	argIdx := 1
	if q.DefID != "" {
		where += fmt.Sprintf(" AND def_id = $%d", argIdx); argIdx++
	...
	if q.StartAfter != nil {
		where += fmt.Sprintf(" AND started_at >= $%d", argIdx); argIdx++
	...
	dataArgs = append(dataArgs, q.Limit, q.Offset)
	rows, err := s.db.QueryContext(ctx,
		fmt.Sprintf("SELECT id, def_id, state, variables, started_at, ended_at, parent_process_instance_id, parent_activity_id FROM process_instances %s ORDER BY started_at DESC LIMIT $%d OFFSET $%d", where, argIdx, argIdx+1),
		dataArgs...)
	if err != nil {
		return nil, 0, err
	}
...
func (s *Store) QueryActivities(ctx context.Context, q storage.ActQuery) ([]*instance.ActivityInstance, int, error) {
	...
	if q.ProcessInstanceID != "" {
		where += fmt.Sprintf(" AND process_instance_id = $%d", argIdx); argIdx++
		args = append(args, q.ProcessInstanceID)
	}
	if q.Assignee != "" {
		where += fmt.Sprintf(" AND assignee = $%d", argIdx); argIdx++
		args = append(args, q.Assignee)
	}
	if q.ActivityID != "" {
		where += fmt.Sprintf(" AND activity_id = $%d", argIdx); argIdx++
		args = append(args, q.ActivityID)
	}
	if q.ActivityType != "" {
		where += fmt.Sprintf(" AND activity_type = $%d", argIdx); argIdx++
